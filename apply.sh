...
original_author="p-virex"
original_name="my_test_project"
original_urlname="my_test_project"
original_description="Awesome my_test_project created by p-virex" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/my_test_project "${name}"
...
then
    ...
    echo "then run 'my_test_project' or 'python -m my_test_project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'my_test_project' or 'python -m my_test_project' or 'poetry run my_test_project'"
fi
